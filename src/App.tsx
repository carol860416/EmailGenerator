...
export default function App() {
  ...
  const handleFileUpload = (field: "handouts" | "snapshots", files: FileList | null) => {
    if (!files) return;
    const names = Array.from(files).map(f => f.name);
    setData(prev => ({ ...prev, [field]: [...(prev[field] || []), ...names] }));
  };

  const removeFile = (field: "handouts" | "snapshots", index: number) => {
    const newFiles = [...(data[field] || [])];
    newFiles.splice(index, 1);
    setData(prev => ({ ...prev, [field]: newFiles }));
  };
...
Details: ${details}
Email Owner/Sender: ${data.courseOwner}
...
Always output JSON with 'subject' and 'body' fields.`
      ...
      // Template-based generation fallback (simplified)
...
---
${isEn ? "📅 Date:" : "📅 課程日期："} ${data.courseDate || "TBD"}
${isEn ? "⏰ Time:" : "⏰ 課程時間："} ${data.courseTime || "TBD"}
  ...
  return (
    <div className="flex flex-col h-screen overflow-hidden bg-moxa-light">
      ...
      <main className="flex-1 flex overflow-hidden">
        ...
        <aside className="w-[400px] bg-white border-r border-border overflow-y-auto p-6 flex flex-col gap-6 shrink-0 custom-scrollbar">
          <div className="space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1">課程負責人</label>
                <input 
                  type="text" 
                  placeholder="L&D Team - 您姓名"
                  value={data.courseOwner}
                  onChange={(e) => handleInputChange("courseOwner", e.target.value)}
                  className="w-full px-3 py-2 bg-white border border-border rounded text-sm outline-none focus:border-moxa-green transition-colors"
                />
              </div>
            </div>
            ...
            {data.type === "PRE_COURSE" && (
              <div className="space-y-4 pt-4 border-t border-gray-100">
                <div className="grid grid-cols-3 gap-2">
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1">日期</label>
                    <input 
                      type="text" 
                      placeholder="MM/DD"
                      value={data.courseDate}
                      onChange={(e) => handleInputChange("courseDate", e.target.value)}
                      className="w-full px-2 py-2 bg-white border border-border rounded text-sm outline-none focus:border-moxa-green transition-colors"
                    />
                  </div>
                  <div className="space-y-1 col-span-2 text-right">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1 text-left block w-full">時間與地點</label>
            ...
            {data.type === "CUSTOM" && (
              <div className="space-y-4 pt-4 border-t border-gray-100">
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest pl-1">時間</label>
                    <input 
                      type="text" 
                      placeholder="HH:MM"
                      value={data.courseTime}
